...
class AmityPollDraftAnswerViewHolder(val context: Context, private val removeCallback: (answer: AmityPollDraftAnswerAdapter.DraftAnswer) -> Unit) :
    AmityViewHolder<AmityPollDraftAnswerAdapter.DraftAnswer>(View.inflate(context, CommunityR.layout.amity_item_poll_draft_answer, null)) {
    ...
    private fun updateAnswer(data: AmityPollDraftAnswerAdapter.DraftAnswer) {
        binding.pollLinearLayout.background = ContextCompat.getDrawable(
            ...
            when ((data.data.isEmpty() && !data.allowEmpty) || data.data.length > MAX_ANSWER_LENGTH) {
                true -> CommunityR.drawable.amity_bg_poll_add_draft_answer_error
                false -> CommunityR.drawable.amity_bg_poll_draft_answer
            }
        )

        binding.errorTextView.text = when {
            data.data.isEmpty() && !data.allowEmpty -> context.getString(CommunityR.string.amity_poll_answer_error_empty)
            binding.answerEditText.length() > MAX_ANSWER_LENGTH -> context.getString(CommunityR.string.amity_poll_answer_error_exceed_limit)
            else -> ""
        }
